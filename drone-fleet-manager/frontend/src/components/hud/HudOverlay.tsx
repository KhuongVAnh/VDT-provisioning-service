...
export const HudOverlay: React.FC<HudOverlayProps> = ({ telemetry, activeDroneId }) => {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none select-none overflow-hidden font-mono z-10 flex flex-col justify-between p-3 sm:p-5 text-tactical-cyan dark:text-tactical-cyan">

      {/* 1. TOP BAR: Compass Ribbon & Flight Mode */}
      <div className="flex items-start justify-between">

        {/* Left Flight Mode & Armed Badge */}
        <div className="flex items-center gap-2 bg-slate-950/60 dark:bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-tactical-cyan/30">
          <span className={`flex items-center gap-1 text-xs font-bold ${armed ? 'text-tactical-emerald' : 'text-tactical-amber'}`}>
        ...
        <div className="flex flex-col items-center">
          <div className="flex items-center gap-1.5 bg-slate-950/60 dark:bg-black/60 backdrop-blur-md px-4 py-1 rounded-lg border border-tactical-cyan/40">
        ...
        <div className="flex items-center gap-3 bg-slate-950/60 dark:bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-tactical-cyan/30 text-xs">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-slate-400">({voltage.toFixed(1)}V)</span>
          </div>
        </div>

      </div>

      {/* 2. CENTER: Pitch & Roll Horizon Ladder + Reticle */}
      <div className="relative flex-1 flex items-center justify-center my-4">
        ...
        >
          ...
          >
            {/* +20 deg */}
            <div className="w-24 h-0.5 border-t-2 border-dashed border-tactical-cyan/60 flex justify-between text-[9px] -mt-3">
              <span>+20</span><span>+20</span>
            </div>
            {/* +10 deg */}
            <div className="w-32 h-0.5 border-t-2 border-tactical-cyan/80 flex justify-between text-[10px] -mt-2">
              <span>+10</span><span>+10</span>
            </div>
            {/* 0 deg Horizon line */}
            <div className="w-44 h-0.5 bg-tactical-cyan shadow-glow-cyan shadow-tactical-cyan/50" />
            {/* -10 deg */}
            <div className="w-32 h-0.5 border-b-2 border-tactical-cyan/80 flex justify-between text-[10px] -mt-2">
              <span>-10</span><span>-10</span>
            </div>
            {/* -20 deg */}
            <div className="w-24 h-0.5 border-b-2 border-dashed border-tactical-cyan/60 flex justify-between text-[9px] -mt-3">
              <span>-20</span><span>-20</span>
            </div>
          </div>
        </div>
        ...
        <div className="absolute left-2 sm:left-6 flex items-center gap-1 bg-slate-950/60 dark:bg-black/60 backdrop-blur-md p-2 rounded-lg border border-tactical-cyan/30">
        ...
        <div className="absolute right-2 sm:right-6 flex items-center gap-1 bg-slate-950/60 dark:bg-black/60 backdrop-blur-md p-2 rounded-lg border border-tactical-cyan/30">
      ...
      {/* 3. BOTTOM BAR: Target Info & Attitude Values */}
      <div className="flex items-center justify-between text-xs bg-slate-950/60 dark:bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-tactical-cyan/30">
        ...
        <div className="flex items-center gap-4 font-mono text-[11px]">
          <div>
            ...
            <span className="font-bold text-tactical-cyan tabular-nums">{roll.toFixed(1)}°</span>
          </div>
          <div>
            <span className="text-slate-400">PITCH: </span>
            <span className="font-bold text-tactical-cyan tabular-nums">{pitch.toFixed(1)}°</span>
          </div>
          <div>
            <span className="text-slate-400">YAW: </span>
            <span className="font-bold text-tactical-cyan tabular-nums">{yaw.toFixed(1)}°</span>
          </div>
        </div>
      </div>
